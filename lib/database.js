...
const REPORT_STATE_ALL = CLOUD_FUNCTIONS_BASE + "/reportstate/alldevices";
const REPORT_STATE = CLOUD_FUNCTIONS_BASE + "/reportstate/singledevice";
...
const GET_CONFIGURATION = CLOUD_FUNCTIONS_BASE + "/api/getconfiguration";
const GET_CLIENT_FUNCTIONS = CLOUD_FUNCTIONS_BASE + "/clientfunctions/getclientfunctions";
...
exports.deleteUserAccount = async function deleteUserAccount() {
  all_tokens = {};
  return await getCloudFunction(DELETE_USER_ACCOUNT);
}

exports.getConfiguration = async function getConfiguration() {
  return await getCloudFunction(GET_CONFIGURATION);
}
...
exports.getServerFeatureLevel = async function getServerFeatureLevel() {
  return await getCloudFunction(GET_SERVER_FEATURELEVEL);
}

exports.getSyncFeatureLevel = async function getSyncFeatureLevel() {
  return await getCloudFunction(GET_SYNC_FEATURELEVEL);
}

exports.reportState = async function(device) {
  console.log('reportstate: ' + device);
  return await postCloudFunction(REPORT_STATE, JSON.stringify({device: device}));
};

exports.reportStateAll = async function() {
  console.log('reportstateall initiated');
  return await getCloudFunction(REPORT_STATE_ALL);
};

exports.initiateSync = async function() {
  return await postCloudFunction(INIT_SYNC);
}

exports.generateMappings = async function() {
  return await getCloudFunction(SYNC_FINISHED);
};
