...
exports.getConfiguration = async function getConfiguration() {
  var res = await fetch(GET_CONFIGURATION, {
    headers: {
      'Authorization': 'Bearer ' + all_tokens.access,
      'content-type': 'application/json'
    }
  ...
  if (res.status == 401) {
    await refreshAllTokens();
    res = await fetch(GET_CONFIGURATION, {
      headers: {
        'Authorization': 'Bearer ' + all_tokens.access,
        'content-type': 'application/json'
      }
...
exports.reportState = async function(device) {
  var res = await fetch(REPORT_STATE, {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + all_tokens.access,
      'content-type': 'application/json'
