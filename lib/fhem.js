...
module.exports = {
    ...
    FHEM_getClientFunctions: FHEM_getClientFunctions
};
...
async function FHEM_getClientFunctions() {
  var fcts = await database.getClientFunctions();
  for (var f in fcts) {
    var loadFctStr = 'global.' + f + '=' + fcts[f];
    eval(loadFctStr);
  }
}
...
FHEM.prototype.connect = async function (callback, filter) {
    ...
    if (response.statusCode === 200) {
      ...
      if (json['totalResultsReturned']) {
        ...
        setTimeout(database.reportStateAll, 40000);
      }
