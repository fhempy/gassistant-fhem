...
//KEEP
async function
FHEM_update(device, reading, readingSetting, orig) {
    if (orig === undefined)
        return;
    
    var informid = device + '-' + reading;

    if (!FHEM_devReadingVal[informid] || orig !== FHEM_devReadingVal[informid]) {
      await database.updateDeviceReading(device, reading, orig);
      console.log('update reading: ' + device + ':' + reading + ' = ' + orig);
      FHEM_devReadingVal[informid] = orig;
    }
    
    //readingSetting.format = float0.5
    //FIXME ReportState only when connected
    //FIXME implement compare function in clientapi.js on server side
    //FIXME implement algo for reportstate (compare to values used in reportstateall)
}
...
//console.log( 'value: ' + value );
                ...
                if (FHEM_deviceReadings.hasOwnProperty(device) && FHEM_deviceReadings[device].hasOwnProperty(reading)) {
                  var readingSetting = FHEM_deviceReadings[device][reading];
                  await FHEM_update(device, reading, readingSetting, value);
        ...
        json['Results'].map(function (s) {
          for (var reading in s.Readings) {
            if (FHEM_deviceReadings.hasOwnProperty(s.Internals.NAME) && FHEM_deviceReadings[s.Internals.NAME].hasOwnProperty(reading)) {
              FHEM_update(s.Internals.NAME, reading, FHEM_deviceReadings[s.Internals.NAME][reading].format, s.Readings[reading].Value);
            }
          }
        }.bind(this));
      }
