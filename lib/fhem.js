...
//KEEP
async function
FHEM_update(device, reading, orig) {
    if (orig === undefined)
        return;
...
async function setInformIdsFromFirestore() {
  ...
  FHEM_activeDevices = {};
  var dev;
  var ref = await database.realdb.ref('/users/' + database.getUid() + '/devices/').once('value');
  ref.forEach(function(child) {
    //device
    console.log('handling ' + child.key);
    child.forEach(function(reading) {
      //reading
      if (reading.key !== 'XXXDEVICEDEFXXX') {
        console.log(' - ' + reading.key);
        FHEM_informids[child.key + '-' + reading.key] = 1;
      }
    });
  });
}
...
//console.log( 'value: ' + value );
                ...
                if (FHEM_informids[informid]) {
                  await FHEM_update(device, reading, value);
