...
FHEM_update(informId, device, orig) {
    ...
    var update = false;
    if (FHEM_informidVal[informId] === undefined) {
      update = true;
    } else {
      if (FHEM_informidVal[informId].value != orig) {
        if (FHEM_informids[informId].format === 'float0.5' &&
            roundHalf(FHEM_informidVal[informId].value) != NaN &&
            roundHalf(orig) != NaN) {
          if (roundHalf(FHEM_informidVal[informId].value) != roundHalf(orig)) {
            update = true;
          }
        } else {
          update = true;
    ...
    if (update) {
      if (FHEM_informidVal[informId] !== undefined) {
        ...
        if ((FHEM_informidVal[informId].time + 30000) > Date.now()) {
          ...
          if (FHEM_informidVal[informId].counter > 3) {
            //mehr als 3 Updates innerhalb der letzten 30s
            return;
          } else {
...
async function setInformIdsFromFirestore() {
  //FIXME change to database.getUsedReadings() - call cloud function
  //retrieve informids
  FHEM_informids = {};
  FHEM_activeDevices = {};
  ...
  var refDefs = await database.realdb.ref('/users/' + database.getUid() + '/informiddefs/').once('value');
  refDefs.forEach(function(child) {
    FHEM_informids[child.key] = {reading: child.val().reading, format: child.val().format};
    if (child.val().format !== '')
      console.log(child.key + ' format: ' + child.val().format);
  });
